...
class RepositoryOverviewViewModel {
    ...
    init(initialLanguage: String, service: GithubManager = GithubManager()) {
        ...
        self.repositories = Observable.combineLatest(_reload, _currentLanguage) { _, language in language }.flatMapLatest { language in
//            service.getMostPopularRepositories(byLanguage: language).catchError { error in
